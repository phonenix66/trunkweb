        ...
        page = {
            init: function (request) {
                ...
                $(".btn_add").on("click", function () {
                    var action_type = $(this).data("tab");
                    console.log(action_type)
                    var title = "新增 " + getTabTitle(action_type) + " 招、投标信息"
                    var type = $(this).data("tab")
                    var $html = _.template(getTempl(action_type))()
                    openForm(action_type,title, type, $html, function (data) {
                        data.pjId = pjId,
                            data.bidType = getUnitType(action_type),
                            data.isNewRecord = true,
                            // data.bidMoney=data.bidMoney*10000,
                            $.api.projectInfo.bid.edit.exec(data, function (re) {
                                buildTable(action_type)
                ...
                $(".btn_edit").on("click", function () {
                    ...
                    if (!obj) return;
                    var title = "修改 " + getTabTitle(action_type) + " 招、投标信息"
                    var type = $(this).data("tab")
                    var $html = _.template(getTempl(action_type))()
                    openForm(action_type,title, type, $html, function (data) {
                        data.pjId = pjId,
                ...
                function openRow(row){
                    var action_type = getActionType(row.bidType);
                ...
                $(".btn_view").on("click", function () {
                    ...
                    var obj = tableHasOneChoose($("#table_" + action_type));
                    if (!obj) return;
                    var title = "查看 " + getTabTitle(action_type) + " 招、投标信息"
                    var type = $(this).data("tab")
                    var $html = _.template(getTempl(action_type))()
                    openForm(action_type,title, type, $html, function (data) {
                                layer.closeAll()
                 ...
                 * @param fun(data) 回调
                 */
                function openForm(action_type,title, type, $html, fun, row,view) {
                    tools.handleModal({
                        title: title,
                        template: $html,
                        area: ["70%", "80%"],
                 ...
                 * @param data 数据
                 */
                function buildTable(action_type) {
                    // action_type 活动tab 刷新
                    var param = dataFormObj("#search_form_" + action_type)
                    param.pjId = pjId,
                        param.bidType = getUnitType(action_type),
                        $.api.projectInfo.bid.dataList.exec(param, function (res) {
                            ...
                            buildSampleTable($("#dataTable_" + action_type)
                                .html('<table id="table_' + action_type + '"></table>').find('table'), data, getClumns(action_type),250,
                                {
                ...
                function getActionType(bidType){
                    switch (bidType) {
                        case "1":return "a";
                        case "2":return "b";
                        case "3":return "c";
                ...
                function getTabTitle(action_type){
                    switch (action_type) {
                        case "a":return "勘察设计单位";
                        case "b":return "监理单位";
                        case "c":return "施工单位";
                        case "d":return "金属结构及其他采购";
                        default:return "未知";
                    }
                }
                buildTable("a")

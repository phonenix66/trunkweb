 ...
 * 站点管理=》站点维护=》站点维护
 */
define("modules/security/aqyhgl/safetrouble", ["jquery", "underscore", "page/tools","layer","bootstrap-table","bootstrap-table-x-editable","bootstrap-datepicker"], function($, _, tools,layer) {
	...
	page = {
		init: function(request) {
			...
			//设置项目id
			if(cityEntity !=null ){
				if(cityEntity != '' && cityEntity.project != null){
			...
			$('.row-body').on("click", '#btn_add', function () {
				if(projectId != ''){
					tools.toDialog({
						name: "增加",
						url: "modules/security/aqyhgl/safetrouble_add.html",
						param: {
							add: true,
							projectId:projectId,

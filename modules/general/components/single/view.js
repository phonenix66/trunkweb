define([
  ...
  './model',
  'text!./tmpl.html',
  'modules/general/components/incident/view',
  'modules/general/components/target/view',
  'bootstrapvalidator'
], function ($, _, Backbone, Model, tmpl, IncidentView, TargetView) {
  'use strict';
  return Backbone.View.extend({
    el: '#singleItemsTmplWrap',
      ...
      this.model.save({
        ...
        if (res.code == 200) {
          layer.open({
            type: 1,
            title: '添加风险事件',
            content: $('#riskIncidentListTmplWrap'),
            area: ['30%', '30%'],
            btn: ['确定', '取消'],
            success: function () {
    ...
    renderTableIncident: function (rows) {
      ...
      this.model.save(subData, {
        ...
        if (res.code == 200) {
          Backbone.trigger('reload:incident', 'edit');
